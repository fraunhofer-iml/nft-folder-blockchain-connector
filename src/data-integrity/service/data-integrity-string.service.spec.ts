...
const STRING_TO_HASH = Buffer.from('test string content');
const ORIGINAL_HASH = crypto.createHash('sha256').update(new Uint8Array(STRING_TO_HASH)).digest('hex');
...
describe('DataIntegrityStringService', () => {
  ...
  describe('hashData', () => {
    it('should hash a string', async () => {
      const hash = service.hashData(STRING_TO_HASH);
  ...
  describe('verifyData', () => {
    it('should verify a string and be true', async () => {
      const valid = service.verifyData(STRING_TO_HASH, ORIGINAL_HASH);
    ...
    it('should throw BadRequestException if data and original hash are not provided', async () => {
    ...
    it('should throw BadRequestException if original hash is not provided', async () => {
      expect(() => service.verifyData(STRING_TO_HASH, null)).toThrow('The original hash must be provided');
    });

    it('should throw BadRequestException if original hash is not a valid SHA-256 hash', async () => {
      expect(() => service.verifyData(STRING_TO_HASH, 'test')).toThrow('The original hash is not a valid SHA-256 hash');
    });
  });
});
